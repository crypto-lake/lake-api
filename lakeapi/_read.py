...
def _read_dfs_from_multiple_paths(
    ...
    def partial_read_func(*args, **kwargs_):
        res = read_func(*args, **{**kwargs, **kwargs_})
        return res

    cpus = ensure_cpu_count(use_threads)
    if cpus < 2 or len(paths) < 2:
        return [
            read_func(
                path,
    ...
    try:
        return thread_map(partial_read_func, paths, versions, max_workers = ensure_cpu_count(use_threads))
    except ImportError:
